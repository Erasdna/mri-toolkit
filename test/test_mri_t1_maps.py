...
def test_looklocker_t1map(tmp_path, mri_data_dir: Path):
    ...
    timestamps = (
        mri_data_dir
        / "mri-dataset/mri_dataset/derivatives/sub-01"
        / "ses-01/anat/sub-01_ses-01_acq-looklocker_IRT1_trigger_times.txt"
    )
    T1_low = 100
    T1_high = 6000
    ...
    looklocker_t1map(
        LL_mri=MRIData.from_file(LL_path, np.single),
        timestamps=timestamps,
        output=test_output_raw,
    )
    looklocker_t1map_postprocessing(
        T1map_mri=MRIData.from_file(test_output_raw, np.single),
        T1_low=T1_low,
        T1_high=T1_high,
        output=test_output,
    )
    compare_nifti_images(
        MRIData.from_file(ref_output, np.single),
        MRIData.from_file(test_output, np.single),
...
def test_mixed_t1map(tmp_path, mri_data_dir: Path):
    ...
    ref_output = mri_data_dir / "mri-dataset/mri_dataset/sub-01" / "ses-01/sub-01_ses-01_acq-mixed_T1map.nii.gz"
    ...
    mixed_t1map(
        SE=MRIData.from_file(SE_path, np.single),
        IR=MRIData.from_file(IR_path, np.single),
        meta_path=meta_path,
        T1_low=T1_low,
        T1_high=T1_high,
        output=test_output_raw,
    ...
    mixed_t1map_postprocessing(
        SE_mri=MRIData.from_file(SE_path, np.single),
        T1_path=test_output_raw,
        output=test_output,
    )
